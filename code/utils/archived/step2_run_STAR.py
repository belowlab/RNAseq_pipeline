# ########## Wanying's notes ##########
# 1. Check the GTEx pipeline:
#     - https://github.com/broadinstitute/gtex-pipeline/blob/master/rnaseq/src/run_STAR.py
# 2. Added new flag --STARalt: allow user-supplied path to STAR, in case STAR is not available from global install
#     - Wanying's STAR (2.7.11, newer than GTEx V10): /data100t1/home/wanying/CCHC/RNAseq_pipeline/tools/STAR-2.7.11b/source/STAR
...
parser.add_argument('--STARalt', default='STAR', help="Wanyig's edits. User supplied path to STAR") # WZ's edit
args = parser.parse_args()

if args.STARlong:
    starcmd = 'STARlong'
else:
    starcmd = args.STARalt # WZ's edit: I have STAR in private install, so needed to specify the path specifically
...
with cd(args.output_dir):
    ...
    if os.path.exists(f'{args.prefix}._STARgenome'): # WZ's edits
